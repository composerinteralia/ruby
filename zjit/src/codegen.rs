...
fn gen_insn(cb: &mut CodeBlock, jit: &mut JITState, asm: &mut Assembler, function: &Function, insn_id: InsnId, insn: &Insn) -> Option<()> {
    ...
    let out_opnd = match insn {
        ...
        Insn::IfPCOffset { expected_pc, target, .. } => return gen_if_pc_offset(jit, asm, *expected_pc, target),
        _ => {
...
fn gen_if_pc_offset(jit: &mut JITState, asm: &mut Assembler, expected_pc: *const u8, branch: &BranchEdge) -> Option<()> {
